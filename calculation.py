...
def calculate(receiver, freq):
    try:
        table1, nrows1 = open_xlsx('Receiver Pseudo Cleaned.xlsx',receiver)
        table2, nrows2 = open_xlsx('Sim Pseudo Cleaned.xlsx',freq)
        ...
        for i in range(0, nrows1 - 1):
            ...
            if table1.row_values(i)[4] == '            ':
                pseB1I = '0'
            else:
                pseB1I = table1.row_values(i)[4]
            if table1.row_values(i)[5] == '            ':
                pseB3I = '0'
            else:
            ...
            for j in range(0, nrows2 - 1):
                ...
                if TOW1 == TOW2 and satnum1 == satnum2:
                    ...
                    txtname = receiver + '_' + freq + '_' + 'C' + str(int(satnum1)) + '.txt'
                    # path = os.getcwd()  
                    f = codecs.open('./Raw/' + txtname, 'a+')
                    f.write(output)
                    f.close()
        return True
...
for i in range(1, 2):
    for j in range(0, 4):
        state = calculate(rxarr[i], freqarr[j])
        if state:
            print('Writing for {} of {} done.'.format(freqarr[j], rxarr[i]))
        else:
            print('Writing for {} of {} ERROR !!!'.format(freqarr[j], rxarr[i]))        